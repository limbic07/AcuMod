...
传统管理器增强阶段已完成：MOD 库已拆出检索工具栏和表格组件，支持持久化显示名称与备注、统一多分类、搜索、筛选、浏览排序、稳定 ID 类别扩展、同模型冲突提示和同类型模型改绑。联网下载和 AI Agent 留在后续阶段。

MOD 库表格重构已完成：最左列使用启用状态按钮，名称和备注首次点击即进入编辑，替换信息使用摘要加展开详情；一个 MOD 可选择多个平等分类，识别生成的初始分类也允许用户编辑或删除。分类只更新元数据，不影响 MOD 文件、当前部署状态或冲突排序。模型修改弹窗只展示分组标签和“替换模型”选择，技术预览改为保存时的内部校验。
...
- `AcumodData/mods/installed/conflict-orders.json`：每个冲突组当前的 MOD 覆盖顺序。
