...
MVP 后的 UI 架构重构已完成：应用具有侧边导航、统一工作区、保留导航的冲突管理页和悬浮式 AI 助手容器，且没有改变既有导入、部署和冲突排序调用链。

传统管理器增强阶段已完成：MOD 库已拆出检索/Profile 工具栏和表格组件，支持持久化显示名称与备注、自动分类、搜索、筛选、浏览排序、稳定 ID 类别扩展、同模型冲突提示和多 Profile。模型替换目标改绑、联网下载和 AI Agent 留在后续阶段。
