...
内置 `15.10.00` 索引可识别武器、防具、发型、随从武器、随从防具、猎虫、挂件、NPC、投射器/飞翔爪和人物语音替换；同一 MOD 可显示多个目标。
...
MOD 库表格重构已完成：最左列使用启用状态按钮，名称和备注首次点击即进入编辑，替换信息使用摘要加展开详情；一个 MOD 可选择多个分类，分类支持顶级和一层子分类，识别生成的初始分类也允许用户编辑或删除。手动拖拽排序只更新 MOD 库浏览顺序，不影响冲突优先级。分类只更新元数据，不影响 MOD 文件、当前部署状态或冲突排序。模型修改弹窗只展示分组标签和“替换模型”选择，技术预览改为保存时的内部校验。

后台任务与进度切片已完成：刷新、导入、启用、禁用、卸载、一键还原、冲突应用和模型改绑均通过统一后台任务入口执行。界面在顶部下方展示当前阶段、文件级进度和耗时；一次刷新用一个工作区快照同时返回 MOD 列表、分类和冲突信息，避免重复读取本地清单。任务不提供取消，重任务按单任务串行执行。

狩技 MOD 盒子的只读扫描和批量导入已实现：导入页读取 `config.ini`、`Mods_582010/<模块 ID>/info.xml` 和 `files/`，展示盒子记录与游戏目录文件比对结果，并默认选择全部模块导入本地库。当前显式“接管已部署文件”原型已被新方案取代，下一切片将移除该入口：导入完成后由后台自动检测实际状态，用户只看到“已启用”“未启用”，以及已启用 MOD 的“部分被冲突覆盖”提示。状态同步不修改游戏目录，也不要求用户逐个选择或确认。

下一切片实施顺序已确定：先成组删除显式接管 UI、typed wrapper、command 和 DTO；再加入重复内容关联与 `mod_state_sync` 分析服务；随后接入自动状态写入、观察所得文件的安全删除规则和冲突顺序推导；最后用完整测试矩阵验证完全匹配、可解释部分覆盖、同内容文件、零匹配、覆盖环和外部改动保护。
...
- 当前阶段能力：基于 MHW 文件 ID 表为新导入 MOD 生成初始分类，并支持多分类编辑、搜索、筛选和浏览排序；稳定识别脸型、怪物、噗吱猪服装、家具及玩家/随从附件，并在冲突组中提示相同替换目标。
- 当前 manifest schema 14：保留 schema 10 引入的改绑选择和 schema 13 的 `.evam` 关联识别，新增 `categoryIds[]` 统一多分类；schema 13 的 `categoryOverride` 会在首次读取时与识别分类合并迁移。本地 MOD 原始副本不改写，有效部署计划统一驱动复制和冲突检测，并精确修正 MRL3 贴图路径与 EVAM 飞翔爪绑定。
