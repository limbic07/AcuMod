...
狩技 MOD 盒子的只读扫描、批量导入和自动状态同步已实现：导入页读取 `config.ini`、`Mods_582010/<模块 ID>/info.xml` 和 `files/`，默认选择全部模块导入本地库。导入时先按已保存的盒子来源关联，再按部署路径与完整文件内容关联重复 MOD；同名但内容不同的模块会独立保存。导入完成后，后台比较本地有效文件与游戏目录，自动显示“已启用”或“未启用”；已启用 MOD 可附带“部分被冲突覆盖”提示。状态同步只写 Acumod 元数据，不修改游戏目录，也不要求用户逐个选择或确认；用户可随时手动重新检测。
