...
- 当前 manifest schema 15：保留 schema 10 引入的改绑选择、schema 13 的 `.evam` 关联识别和 schema 14 的 `categoryIds[]` 多分类；新增盒子来源别名 `legacySources[]`、已启用的部分覆盖提示，以及每条 `deployedFiles[]` 的 `deploymentOrigin`。本地 MOD 原始副本不改写，有效部署计划统一驱动复制和冲突检测，并精确修正 MRL3 贴图路径与 EVAM 飞翔爪绑定。
