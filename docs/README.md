...
当前已完成到第十个切片。除启用、禁用、卸载、一键还原和按冲突组排序外，窗口任意位置可拖入一个 MOD 文件夹或 `.zip`、`.7z`、`.rar` 压缩包，确认后导入；出现多个同级 MOD 内容根时，界面会列出候选版本，用户选择一个分支后只导入该分支。同名 MOD 返回已有记录，不重复安装。导入时还会根据内置 `15.10.00` 模型索引识别武器和防具替换目标，并在导入结果和已安装 MOD 列表显示类型、模型 ID 和游戏名称；同一 MOD 可显示多个目标。
...
- MHW 模型替换识别：`references/mhwi-data/curated/model-index.json` 内置维护武器、防具和发型模型索引；发型优先显示数字槽位，名称使用官方简体中文，未知编号按资源路径显示 ID。第一版只识别，不提供替换目标选择或改绑。
